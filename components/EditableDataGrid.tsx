...
export function EditableDataGrid({ data, onDataChange }: EditableDataGridProps) {
  ...
  const handleCellChange = (id: string, field: keyof DataRow, value: string) => {
    onDataChange(data.map(row => 
      row.id === id ? { ...row, [field]: value } : row
    ));
  };

  const handlePaste = (e: React.ClipboardEvent, rowIndex: number, field: keyof DataRow) => {
    e.preventDefault();
    ...
    if (rows.length === 1) {
      ...
      if (cells.length === 1) {
        ...
        const fields: (keyof DataRow)[] = ['orderNo', 'itemCode', 'externalLot', 'materialCode', 'internalLot', 'qty'];
  ...
  return (
    <div className="bg-white rounded-lg border-2 border-gray-300 overflow-hidden">
      {/* Header */}
      <div className="grid grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_50px] bg-gradient-to-r from-yellow-400 to-yellow-500 border-b-2 border-gray-400">
        <div className="px-4 py-3 font-bold text-gray-800 border-r border-gray-400">ORDER NO.</div>
        <div className="px-4 py-3 font-bold text-gray-800 border-r border-gray-400">Item Code</div>
        <div className="px-4 py-3 font-bold text-gray-800 border-r border-gray-400">External Lot.</div>
        <div className="px-4 py-3 font-bold text-gray-800 border-r border-gray-400">Material code</div>
        <div className="px-4 py-3 font-bold text-gray-800 border-r border-gray-400">Internal Lot</div>
        <div className="px-4 py-3 font-bold text-gray-800 border-r border-gray-400">Q'ty</div>
      ...
      <div className="max-h-[400px] overflow-y-auto">
        {data.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm">You can also paste data from Excel directly into the cells.</p>
          </div>
        ) : (
          data.map((row, index) => (
            <div key={row.id} className="grid grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_50px] border-b border-gray-200 hover:bg-gray-50">
              <input
                type="text"
                value={row.orderNo}
                onChange={(e) => handleCellChange(row.id, 'orderNo', e.target.value)}
              ...
              <input
                ...
                onChange={(e) => handleCellChange(row.id, 'externalLot', e.target.value)}
              ...
              <input
                ...
                onChange={(e) => handleCellChange(row.id, 'materialCode', e.target.value)}
              ...
              <input
                ...
                onChange={(e) => handleCellChange(row.id, 'internalLot', e.target.value)}
